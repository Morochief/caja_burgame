    ...
    if (!_matches || _matches.length === 0) {
        return `
            <div style="text-align: center; padding: 3rem 1rem; color: var(--text-muted); width: 100%;">
                ...
                <p style="font-size: 0.85rem;">Las llaves oficiales se publicarán una vez cerradas las inscripciones.</p>
            </div>
        `;
    }

    // Agrupar partidos por round_index
    const roundsMap = {};
    _matches.forEach(m => {
        ...
        return `
            <div class="bracket-round-column">
                ...
                <div class="bracket-round-matches">
                    ${matchesInRound.map(m => renderMatchCardHtml(m)).join('')}
                </div>
            </div>
        `;
    }).join('');
    ...
    return `
        <div class="match-card ${isCompleted ? 'match-card--completed' : ''} ${isLive ? 'match-card--in_progress' : ''}">
            <div class="match-header">
                ...
                <span>${isCompleted ? 'Finalizado' : isLive ? '🔥 EN JUEGO' : 'Pendiente'}</span>
            </div>

            <!-- Equipo 1 -->
            <div class="match-team ${p1Winner ? 'match-team--winner' : (isCompleted && !p1Winner ? 'match-team--loser' : '')}">
                <span class="match-team-name ${!p1 ? 'match-team-name--empty' : ''}">${p1Name}</span>
            ...
            <div class="match-team ${p2Winner ? 'match-team--winner' : (isCompleted && !p2Winner ? 'match-team--loser' : '')}">
                <span class="match-team-name ${!p2 ? 'match-team-name--empty' : ''}">${p2Name}</span>
